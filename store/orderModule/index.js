...
const invokeGetOrderByStatusApi = async ({ pageIndex, status, hasZero, cancelToken }) => {
	const {data} = await getMyOrderList({ pageIndex, status, hasZero, flagReceipt: 0, istrue:true }, cancelToken)
...
export default {
	...
	actions: {
		async getUnpaiedOrderAction({commit, state}, { pageIndex, reset, cancelToken }) {
			if (reset) commit('resetUnpaidOrderDetail')
			const mappedOrders = await invokeGetOrderByStatusApi({ pageIndex, status: 3, hasZero: 2, cancelToken })
			Array.isArray(mappedOrders) ? commit('afterLoadUnpaidOrders', mappedOrders) : commit('resetUnpaidOrderDetail')
		},
		async getOngoingOrders({commit, state},{ pageIndex, hasZero, reset, cancelToken }) {
			if (reset) commit('resetOngoingOrderDetail')
			const mappedOrders = await invokeGetOrderByStatusApi({ pageIndex, status: 1, hasZero, cancelToken })
			Array.isArray(mappedOrders) ? commit('afterLoadOngoingOrders', mappedOrders) : commit('resetOngoingOrderDetail')
		},
		async getFinishedOrders({commit, state},{ pageIndex, hasZero, reset, cancelToken }) {
			if (reset) commit('resetFinishedOrderDetail')
			const mappedOrders = await invokeGetOrderByStatusApi({ pageIndex, status: 2, hasZero, cancelToken })
