	...
	<view>
		...
		<view class="pdb160">
					...
					<view class="fs">
						<view  v-for="(item,index) in radioList" :key='index' @click="checkType(item)">
							<div class="fi mgr30">
								...
								<image class='img' v-else :src="staticImg + 'static/img/X_select_0.png'" ></image>
			...
			<view class="pd40-x pd30-y fst bgw border">
				...
				<view class="rgb74 ft24 flex1 fs" @click="checkHead" v-if="headInfo.name">	
					<view >{{headInfo.name}}</view>
					<uni-icons type="arrowright" size="24" class="mgt4 arrowRight rgb170" color="#AAAAAA"></uni-icons>
				</view>
				<view class="rgb74 ft24 flex1 fs" @click="checkHead" v-else>		
					<view class="placeholderClass">请选择抬头</view>
					<uni-icons type="arrowright" size="24" class="mgt4 arrowRight rgb170" color="#AAAAAA"></uni-icons>
				</view>
			</view>
			<view class="pd40-x pd30-y fst bgw" v-if="form.companyType == 1">
				<view class="rgb27 ft28 ftweight mgr40">税号</view>
				<view class="rgb74 ft24 flex1"><input type="text" :value="headInfo.number" placeholder="请填写企业识别号" placeholder-class="placeholderClass"/></view>
			</view>
			<view class="pd40-x pd30-y fst bgw mgt16 border">
				<view class="rgb27 ft28 ftweight mgr40">发票金额</view>
				<view class="rgb74 ft24 flex1"><input type="text" value="" placeholder="请填写发票金额" placeholder-class="placeholderClass"/></view>
			</view>
			<view class="pd40-x pd30-y fst bgw">
		...
		<view class="bottom fs">
			...
			<view class="btn savebtn" @click="save">申请开票</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onUnload() {
		   store.commit("set_headInfo",null)
		},
		methods: {
			checkType(obj){
			...
			checkHead(){
				uni.navigateTo({
					url:'/personal/invoicing/headManage/index1?type=apply'
				})
			}
