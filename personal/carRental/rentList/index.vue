...
<div>
	...
	<div class="pd50-y pd40-x fs bgw top" style="width:100%;box-sizing:border-box;z-index:2;">
      <div class="ft50 ftweight rgb27">租赁订单</div> 
    </div>
	<!-- 列表 -->
	<mescroll-uni class="mescroll" ref="mescrollRef" 
		...
		<view class="card mgt50 bgw pd20-y mg40-x" v-for="(order,index) in rentListData" :key="index" @click="nav2orderDetail(index)">
			<view class="fs mgb20 pdb20 pd40-x"  style="border-bottom:2rpx solid rgba(239,244,248,1);" :class="order.orderStatus==3?'gray':''">
				<view class="fst">
					<view class="ftweight ft32 rgb27  ellipsis">{{order.parkingName}}</view>
					<view class="mgl20 fst">
						<view class="ft14 ftweight type">{{order.cardType == 1?'周卡': order.cardType == 2?'月卡':'年卡'}}</view>
					</view>
				</view>
				<view class="ftweight ft20" :class="order.orderStatus == 1?'Leasing':order.orderStatus == 3?'overdue':'noeffective'">{{order.orderStatus == 1?'租赁中':order.orderStatus == 3?'已到期':'待生效'}}</view>						
			</view>
			<view class="fs pd40-x" style="align-items: flex-end;">
				<view style="width:80%;" :class="order.orderStatus==3?'gray':''">
					<view class="fst pdl30 address" >
						<view class="ellipsis ft20 rgb74">{{order.address}}</view>
					</view>
					<view class="ft20 rgb74  parkIcon pdl30 mgt12 fst" v-if="order.isFixedFlag">
...
computed: {
	...mapState('carRentalModule/payModule',{
		rentList: (state) => state.rentList,
		rentListData: (state) => state.rentList.rentListData,
		isEndSuccess: (state) => state.rentList.isEndSuccess,
		currentRentListData:  (state) => state.rentList.currentRentListData,
	}),
	pageIndex: {
		get() {
			return this.rentList.pageIndex
		},
		set(pageIndex) {
			this.rentList.pageIndex = pageIndex
		}
	},
},
...
methods: {
	...mapActions('carRentalModule/payModule',['getRentOrderListAction']),
	downCallback() {
	...
	async upCallback(page) {
		...
		this.getRentOrderListAction(page.num).then(() => {
			this.mescroll.endSuccess(this.currentRentListData.length,this.isEndSuccess)
		})
	}
},
}
