...
<script>
	...
	import {getBookingGoodsInfo} from '@/api'
	export default {
		props:{
			tabStatus: Number,
			// checkMode: Boolean,
		...
		methods:{
			...
			async btnClick(){
				if(this.tabStatus == 1){
					...
					// 跳转到车检详情页面
					const {data} = await getBookingGoodsInfo({
						 id: this.order.goodsId //商品id
					})
					if(data.code==1 ){
						goLink( this.$store.state.h5Url, {
							path: '#/pages/carCheck/fillOrder?id=' + this.order.goodsId + '&isAgain=1',
							token: this.$store.state.token
						})
					}else {
						uni.showToast({
...
.arrowPay
	...
	background-position left center
.arrowPuchase
	background url(@/assets/orderIconPuchase.png) no-repeat
	background-size 20rpx 22rpx
	background-position left center
.arrowRealPay
